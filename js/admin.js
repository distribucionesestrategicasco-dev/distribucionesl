...
function guardarProducto() {
  ...
  fetch(SHEETS_URL, {
    method: 'POST', mode: 'no-cors',
    headers: { 'Content-Type': 'text/plain' },
    body: JSON.stringify({ action: 'saveProduct', id: idVal || 'new', name, cat, icon, price, img }),
  }).catch(err => console.warn(err));
...
function renderCatalogo() {
  ...
  var catBtns = [...cats, 'Todos'].map(function(c) {
  ...
  var rows = filtrado.length === 0
    ...
    : filtrado.map(function(p) {
        ...
        return '<tr style="' + (!activo ? 'opacity:0.45' : '') + '">'
          + '<td style="width:56px">'
          + (p.imagen_url
            ? '<img src="' + p.imagen_url + '" alt="' + p.nombre + '" style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid var(--border)">'
          ...
          + '<td style="font-size:13px;color:var(--text-soft)">' + (p.precio_ref ? '$' + fmt(p.precio_ref) : 'Por cotizar') + '</td>'
          ...
          + (isAdmin ? '<td>'
            ...
            + '<button class="action-link" style="color:' + (!activo ? 'var(--brand-blue)' : '#A32D2D') + '" onclick="toggleProductoSupa(\'' + p.id + '\',' + activo + ')">' + (!activo ? '✅ Activar' : '⛔ Desactivar') + '</button>'
            + '</td>' : '')
          + '</tr>';
      }).join('');
  ...
  var modal = '<div id="prod-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:400;align-items:center;justify-content:center;padding:20px">'
    + '<div style="background:var(--bg-white);border-radius:16px;padding:32px;width:100%;max-width:520px;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,0.25)">'
    + '<h3 id="prod-modal-title" style="font-size:20px;font-weight:800;margin-bottom:20px">Nuevo Producto</h3>'
    + '<input type="hidden" id="prod-id" value="">'
    + '<div class="form-group"><label>Nombre *</label><input id="prod-name" placeholder="Nombre del producto"></div>'
    + '<div class="form-group"><label>Categoria *</label>'
    + '<select id="prod-cat"><option>Oficina</option><option>Papeleria</option><option>Tecnologia</option><option>Equipos</option><option>Otros</option></select>'
    + '</div>'
    + '<div class="form-group"><label>Icono (emoji)</label><input id="prod-icon" placeholder="📦" maxlength="4"></div>'
    + '<div class="form-group"><label>Precio referencia</label><input id="prod-price" type="number" placeholder="0" min="0"></div>'
    + '<div class="form-group"><label>URL imagen (opcional)</label><input id="prod-img" placeholder="https://..."></div>'
    + '<div style="display:flex;gap:12px;margin-top:8px">'
    + '<button onclick="guardarProductoSupa()" style="background:var(--brand-blue);color:#fff;border:none;padding:12px 24px;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;flex:1">💾 Guardar</button>'
    + '<button onclick="document.getElementById(\'prod-modal\').style.display=\'none\'" style="background:var(--bg);border:none;padding:12px 24px;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer">Cancelar</button>'
    + '</div></div></div>';
  ...
  return '<div class="admin-header"><div><h1>Catalogo de Productos</h1><p>' + filtrado.length + ' de ' + _catalogoSupa.length + ' productos</p></div>'
    ...
    + '<table><thead><tr><th>Icono</th><th>Nombre</th><th>Categoria</th><th>Precio ref.</th><th>Estado</th>' + (isAdmin ? '<th>Acciones</th>' : '') + '</tr></thead>'
...
function guardarProductoSupa() {
  ...
  var SUPA_URL  = 'https://jnxsofraqshxjboukiab.supabase.co';
  var SUPA_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpueHNvZnJhcXNoeGpib3VraWFiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM2NjkxNzUsImV4cCI6MjA4OTI0NTE3NX0.CejqobwjHcbrgnT7nn29dgYzLf-bLT_J0fqDvvb59Gs';
  var body    = { nombre: nombre, categoria: cat, icono: icono, precio_ref: precio, imagen_url: img || null };
...
function toggleProductoSupa(id, activo) {
  ...
  .then(function(r) {
    if (r.ok) {
      showAdminToast(!activo ? '✅ Producto activado' : '⛔ Producto desactivado');
      loadCatalogoSection(document.getElementById('admin-content'));
    } else {
