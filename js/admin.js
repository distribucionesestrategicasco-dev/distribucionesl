...
function doLogin() {
  ...
  .then(function(data) {
    ...
    if (data && data.length > 0 && data[0].password === p) {
      ...
      try { localStorage.setItem('dlc_session', JSON.stringify(currentUser)); } catch(e) {}
      showPage('admin');
...
function renderUsuarios(users) {
  ...
  var modal = '<div id="user-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:400;align-items:center;justify-content:center;padding:20px">'
  ...
  return '<div class="admin-header"><div><h1>Usuarios</h1><p>' + users.length + ' usuario(s)</p></div>'
    ...
    + (users.length === 0
      ...
      : '<table><thead><tr><th>Usuario</th><th>Nombre</th><th>Rol</th><th>Estado</th>'
        + (isAdmin ? '<th>Acciones</th>' : '') + '</tr></thead><tbody>' + rows + '</tbody></table>')
    + '</div>' + modal;
}
