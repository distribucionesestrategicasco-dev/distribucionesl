...
function generarPDFCotizacion(orderId) {
  ...
  var items = o.items.map(function(i, idx) {
    ...
    return '<tr class="' + (idx % 2 === 0 ? 'even' : 'odd') + '">'
      + '<td style="padding:10px 12px;font-size:13px;font-weight:600;color:#1A1208">' + _esc(i.name) + '</td>'
      + '<td style="padding:10px 12px;font-size:13px;text-align:center;font-weight:700;color:#9A7535">' + i.qty + '</td>'
      + '<td style="padding:10px 12px;font-size:13px;text-align:right;color:#5C4A1E">' + (prec ? '$' + fmt(prec) : '—') + '</td>'
      + '<td style="padding:10px 12px;font-size:13px;text-align:right;font-weight:700;color:#1A1208">' + (subt ? '$' + fmt(subt) : '—') + '</td>'
      + '</tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">'
    ...
    + '@media print{.no-print{display:none}@page{margin:1.2cm}}'
    + '.header{background:linear-gradient(135deg,#0A0806 0%,#1A1208 100%);padding:22px 36px;display:flex;justify-content:space-between;align-items:center}'
    + '.logo{height:56px}'
    + '.gold-line{height:3px;background:linear-gradient(90deg,transparent,#E2C47A 25%,#C8A558 75%,transparent)}'
    + '.meta{background:#FAF7F0;padding:18px 36px;display:grid;grid-template-columns:1fr 1fr;gap:10px 40px;border-bottom:1px solid #EDE5C8}'
    + '.meta-item label{font-size:10px;font-weight:700;text-transform:uppercase;color:#9A7535;letter-spacing:1.2px}'
    + '.meta-item span{font-size:13px;font-weight:600;display:block;margin-top:2px;color:#1A1208}'
    + 'table{width:100%;border-collapse:collapse;margin:0 36px;width:calc(100% - 72px)}'
    + 'thead tr{background:linear-gradient(135deg,#0A0806,#1A1208)}'
    + 'th{padding:9px 12px;font-size:10px;color:#E2C47A;text-align:left;font-weight:700;letter-spacing:1px}'
    + 'tr.even{background:#FDFAF3}tr.odd{background:#F5EED8}'
    + 'td{border-bottom:1px solid #EDE5C8}'
    + '.totals{padding:16px 36px;display:flex;justify-content:flex-end}'
    + '.totals-box{background:#FAF7F0;border:1px solid #EDE5C8;border-left:3px solid #C8A558;border-radius:8px;padding:12px 18px;min-width:220px}'
    + '.totals-row{display:flex;justify-content:space-between;font-size:13px;color:#9A7535;padding:3px 0}'
    + '.totals-total{display:flex;justify-content:space-between;font-size:15px;font-weight:800;color:#1A1208;border-top:1px solid #C8A558;padding-top:8px;margin-top:4px}'
    + '.footer{background:#0A0806;color:rgba(226,196,122,0.50);padding:10px 36px;font-size:10px;display:flex;justify-content:space-between;margin-top:24px}'
    + '.no-print-bar{padding:14px 36px;background:#FAF7F0;border-bottom:2px solid #EDE5C8;display:flex;gap:10px}'
    + '</style></head><body>'
    ...
    + '</div>'
    + '<div class="header">'
    + (logoSrc ? '<img class="logo" src="' + logoSrc + '" alt="DLC">' : '<div style="color:#E2C47A;font-size:24px;font-weight:900">DLC</div>')
    + '<div style="text-align:right">'
    + '<div style="background:rgba(200,165,88,0.18);border:1px solid rgba(200,165,88,0.45);border-radius:6px;padding:4px 12px;display:inline-block;margin-bottom:6px"><span style="color:#E2C47A;font-size:10px;font-weight:800;letter-spacing:1.5px">REMISIÓN DE DESPACHO</span></div>'
    + '<div style="color:#fff;font-size:16px;font-weight:800;letter-spacing:-0.3px">N° ' + o.id + '</div>'
    + '<div style="color:rgba(226,196,122,0.60);font-size:10px;margin-top:2px">Fecha: ' + today + '</div>'
    + '</div></div>'
    ...
    + '</div>'
    + '<div style="padding:16px 36px 8px"><table><thead><tr>'
    ...
    + '</div></div>'
    + (o.notes ? '<div style="margin:0 36px 16px;padding:12px 16px;background:#FAF7F0;border-radius:8px;border-left:3px solid #C8A558"><div style="font-size:10px;font-weight:700;color:#9A7535;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Observaciones</div><div style="font-size:13px;color:#5C4A1E">' + _esc(o.notes) + '</div></div>' : '')
    + '<div class="footer"><span>Distribuciones Estratégicas de la Costa S.A.S · distribucionesestrategicasco@gmail.com · +57 302 354 8415</span><span>' + o.id + '</span></div>'
    + '</body></html>';
...
function _buildRemisionHTML(datos) {
  ...
  var filas=items.map(function(item,i){
    ...
    return '<tr style="background:'+(i%2===0?'#FDFAF3':'#F5EED8')+'">'
      +'<td style="padding:8px 10px;font-size:11px;color:#9A7535;border-bottom:1px solid #EDE5C8;text-align:center">'+(i+1)+'</td>'
      +'<td style="padding:8px 10px;font-size:13px;font-weight:600;color:#1A1208;border-bottom:1px solid #EDE5C8">'+_esc(item.name)+'</td>'
      +'<td style="padding:8px 10px;font-size:13px;font-weight:800;color:#9A7535;text-align:center;border-bottom:1px solid #EDE5C8">'+item.qty+'</td>'
      +(mostrarPrecios?'<td style="padding:8px 10px;font-size:12px;text-align:right;border-bottom:1px solid #EDE5C8;color:#5C4A1E">$'+fmt(item.price||0)+'</td>':'')
      +(mostrarPrecios?'<td style="padding:8px 10px;font-size:12px;font-weight:700;text-align:right;border-bottom:1px solid #EDE5C8;color:#1A1208">$'+fmt(sub2)+'</td>':'')
      +'<td style="padding:8px 10px;border-bottom:1px solid #EDE5C8;text-align:center"><div style="width:18px;height:18px;border:1.5px solid #C8A558;border-radius:3px;margin:0 auto"></div></td>'
    +'</tr>';
  }).join('');
  return '<div id="remision-print" style="font-family:Arial,sans-serif;background:#fff;font-size:13px">'
    +'<div style="background:linear-gradient(135deg,#0A0806 0%,#1A1208 100%);padding:18px 24px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">'
      +'<div style="display:flex;align-items:center;gap:12px">'+logo
        +'<div>'
          +'<div style="font-size:16px;font-weight:800;color:#fff;letter-spacing:-0.3px">Distribuciones Estrat\u00e9gicas</div>'
          +'<div style="font-size:9px;font-weight:700;color:#E2C47A;letter-spacing:2.5px;text-transform:uppercase;margin-top:1px">de la Costa S.A.S</div>'
          +'<div style="font-size:9px;color:rgba(226,196,122,0.55);margin-top:2px">NIT 901.445.281-1</div>'
          +'<div style="font-size:9px;color:rgba(255,255,255,0.38);margin-top:2px">\u260e (57) 302 354 8415 \u00b7 distribucionesestrategicasco@gmail.com</div>'
        +'</div>'
      +'</div>'
      +'<div style="text-align:right">'
        +'<div style="background:rgba(200,165,88,0.18);border:1px solid rgba(200,165,88,0.45);border-radius:6px;padding:5px 12px;margin-bottom:6px;display:inline-block">'
          +'<span style="color:#E2C47A;font-size:10px;font-weight:800;letter-spacing:1.5px">REMISI\u00d3N DE DESPACHO</span>'
        +'</div>'
        +'<div style="color:#fff;font-size:14px;font-weight:800;letter-spacing:-0.3px">N\u00b0 '+remNum+'</div>'
        +'<div style="color:rgba(226,196,122,0.60);font-size:10px;margin-top:2px">Fecha: '+today+'</div>'
      +'</div>'
    +'</div>'
    +'<div style="height:3px;background:linear-gradient(90deg,transparent,#E2C47A 25%,#C8A558 75%,transparent)"></div>'
    +'<div style="padding:12px 24px;background:#FAF7F0;border-bottom:1px solid #EDE5C8">'
      ...
      +'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px 20px">'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">Cliente</div><div style="font-size:12px;font-weight:700;color:#1A1208">'+(_esc(cliente)||'&mdash;')+'</div></div>'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">Empresa</div><div style="font-size:12px;font-weight:700;color:#1A1208">'+(_esc(empresa)||'&mdash;')+'</div></div>'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">NIT / CC</div><div style="font-size:12px;color:#1A1208">'+(_esc(nit)||'&mdash;')+'</div></div>'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">Email</div><div style="font-size:12px;color:#1A1208">'+(_esc(email)||'&mdash;')+'</div></div>'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">Tel\u00e9fono</div><div style="font-size:12px;color:#1A1208">'+(_esc(telefono)||'&mdash;')+'</div></div>'
        +'<div><div style="font-size:9px;color:#9A7535;font-weight:600">Ciudad</div><div style="font-size:12px;color:#1A1208">'+(_esc(ciudad)||'&mdash;')+'</div></div>'
      +'</div>'
    +'</div>'
    +'<div style="padding:12px 24px">'
      ...
      +'<table style="width:100%;border-collapse:collapse;border:1px solid #EDE5C8;font-size:12px">'
        +'<thead><tr style="background:linear-gradient(135deg,#0A0806,#1A1208)">'
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:center;width:28px;letter-spacing:1px">#</th>'
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:left;letter-spacing:1px">Producto</th>'
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:center;width:60px;letter-spacing:1px">Cant.</th>'
          +(mostrarPrecios?'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:right;width:90px;letter-spacing:1px">P. Unit.</th>':'')
          +(mostrarPrecios?'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:right;width:90px;letter-spacing:1px">Subtotal</th>':'')
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:center;width:70px;letter-spacing:1px">Recibido \u2713</th>'
        +'</tr></thead><tbody>'+filas+'</tbody></table>'
    +'</div>'
    +(mostrarTotales&&sub>0
      ?'<div style="padding:0 24px 10px;display:flex;justify-content:flex-end">'
        +'<div style="background:#FAF7F0;border-radius:8px;padding:12px 16px;border:1px solid #EDE5C8;border-left:3px solid #C8A558;min-width:190px">'
          +'<div style="display:flex;justify-content:space-between;font-size:11px;color:#9A7535;margin-bottom:4px"><span>Subtotal</span><span>$'+fmt(sub)+'</span></div>'
          +'<div style="display:flex;justify-content:space-between;font-size:11px;color:#9A7535;margin-bottom:6px"><span>IVA (19%)</span><span>$'+fmt(iva)+'</span></div>'
          +'<div style="display:flex;justify-content:space-between;font-size:13px;font-weight:800;color:#1A1208;border-top:1px solid #C8A558;padding-top:6px"><span>TOTAL</span><span>$'+fmt(total)+'</span></div>'
        +'</div></div>'
      :'')
    +(notas
      ?'<div style="padding:0 24px 10px">'
        +'<div style="background:#FAF7F0;border-radius:8px;padding:10px 14px;border-left:3px solid #C8A558">'
          ...
          +'<div style="font-size:12px;color:#5C4A1E">'+_esc(notas)+'</div>'
        +'</div></div>'
      :'')
    +'<div style="min-height:60px"></div>'
    +'<div class="firmas-block" style="padding:10px 24px 14px;border-top:1px solid #EDE5C8;display:grid;grid-template-columns:1fr 1fr;gap:30px">'
      +'<div style="text-align:center">'
        ...
        +'<div style="border:1.5px solid #C8A558;border-radius:6px;background:#fff;min-height:70px;padding:6px 10px;display:flex;align-items:center;justify-content:center">'
        ...
        +'</div>'
        +'<div style="border-top:1px dashed #C8A558;margin-top:6px;padding-top:4px;font-size:9px;color:#9A7535">Distribuciones Estrat\u00e9gicas de la Costa S.A.S</div>'
      +'</div>'
      +'<div style="text-align:center">'
        ...
        +'<div style="border:1.5px solid #C8A558;border-radius:6px;background:#fff;min-height:70px;padding:6px 10px"></div>'
        +'<div style="border-top:1px dashed #C8A558;margin-top:6px;padding-top:4px;font-size:9px;color:#9A7535">Nombre, C.C. y Sello</div>'
      +'</div>'
    +'</div>'
