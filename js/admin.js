...
function generarPDFCotizacion(orderId) {
  ...
  var items = o.items.map(function(i, idx) {
    ...
    return '<tr class="' + (idx % 2 === 0 ? 'even' : 'odd') + '">'
      + '<td style="padding:9px 10px;font-size:11px;color:#9A7535;text-align:center;border-bottom:1px solid #EDE5C8">' + (idx+1) + '</td>'
      + '<td style="padding:9px 10px;font-size:13px;font-weight:600;color:#1A1208;border-bottom:1px solid #EDE5C8">' + _esc(i.name) + '</td>'
      + '<td style="padding:9px 10px;font-size:13px;text-align:center;font-weight:700;color:#9A7535;border-bottom:1px solid #EDE5C8">' + i.qty + '</td>'
      + '<td style="padding:9px 10px;font-size:13px;text-align:right;color:#5C4A1E;border-bottom:1px solid #EDE5C8">' + (prec ? '$' + fmt(prec) : '—') + '</td>'
      + '<td style="padding:9px 10px;font-size:13px;text-align:right;font-weight:700;color:#1A1208;border-bottom:1px solid #EDE5C8">' + (subt ? '$' + fmt(subt) : '—') + '</td>'
      + '</tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">'
    ...
    + '<style>'
    + 'body{font-family:Arial,sans-serif;padding:0;margin:0;color:#1A1208;background:#fff}'
    + '@media print{.no-print{display:none}@page{size:A4 portrait;margin:1.5cm}}'
    + '.hdr{background:linear-gradient(135deg,#0A0806 0%,#1A1208 100%);padding:24px 32px;display:flex;align-items:center}'
    + '.hdr-left{display:flex;align-items:center;gap:14px;flex:1;padding-right:28px;border-right:2px solid rgba(200,165,88,0.28)}'
    + '.hdr-logo{height:52px}'
    + '.hdr-name{font-size:14px;font-weight:700;color:#fff;letter-spacing:-0.2px;line-height:1.2}'
    + '.hdr-nit{font-size:8px;font-weight:700;color:#E2C47A;letter-spacing:2.5px;text-transform:uppercase;margin-top:2px}'
    + '.hdr-info{font-size:8.5px;color:rgba(255,255,255,0.40);margin-top:4px;line-height:1.7}'
    + '.hdr-right{text-align:right;padding-left:28px;flex-shrink:0}'
    + '.doc-type{font-size:8px;font-weight:800;color:#E2C47A;letter-spacing:2.5px;text-transform:uppercase;margin-bottom:8px}'
    + '.doc-num{font-size:20px;font-weight:900;color:#fff;letter-spacing:-0.5px;line-height:1}'
    + '.doc-date{font-size:10px;color:rgba(226,196,122,0.60);margin-top:5px}'
    + '.band{height:4px;background:#C8A558}'
    + '.cli{display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid #EDE5C8}'
    + '.cli-box{padding:14px 28px}'
    + '.cli-box+.cli-box{border-left:1px solid #EDE5C8}'
    + '.cli-ttl{font-size:8px;font-weight:800;color:#9A7535;text-transform:uppercase;letter-spacing:2px;padding-bottom:6px;margin-bottom:10px;border-bottom:2px solid #C8A558;display:inline-block}'
    + '.cli-row{display:flex;gap:8px;margin-bottom:5px;align-items:baseline}'
    + '.cli-lbl{font-size:8px;font-weight:700;color:#9A7535;text-transform:uppercase;letter-spacing:0.8px;min-width:62px;flex-shrink:0}'
    + '.cli-val{font-size:12px;font-weight:600;color:#1A1208}'
    + '.tbl-hdr{background:#1A1208;padding:7px 28px}'
    + '.tbl-ttl{font-size:8px;font-weight:800;color:#E2C47A;text-transform:uppercase;letter-spacing:2.5px}'
    + '.tbl-wrap{padding:0 28px}'
    + 'table{width:100%;border-collapse:collapse}'
    + 'thead tr{background:#2A1C08}'
    + 'th{padding:9px 10px;font-size:8.5px;font-weight:700;color:#E2C47A;text-align:left;border-bottom:2px solid #C8A558;letter-spacing:0.8px}'
    + 'tr.even{background:#fff}tr.odd{background:#FAFAF8}'
    + '.totals-wrap{padding:0 28px 16px}'
    + '.totals-inner{margin-left:auto;width:260px;border:1px solid #EDE5C8;border-top:none}'
    + '.tot-row{display:flex;justify-content:space-between;padding:6px 12px;font-size:12px;color:#5C4A1E;border-bottom:1px solid #EDE5C8}'
    + '.tot-total{display:flex;justify-content:space-between;padding:9px 12px;font-size:15px;font-weight:900;color:#0A0806;background:#C8A558}'
    + '.notes-box{margin:0 28px 14px;padding:10px 14px;background:#FDFAF8;border-left:3px solid #C8A558;border-radius:0 4px 4px 0}'
    + '.notes-ttl{font-size:8px;font-weight:800;color:#9A7535;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px}'
    + '.footer{background:#0A0806;padding:10px 32px;display:flex;justify-content:space-between;align-items:center;margin-top:8px}'
    + '.ftr-l{font-size:9px;color:rgba(226,196,122,0.50)}'
    + '.ftr-r{font-size:9px;color:rgba(226,196,122,0.30)}'
    + '.no-print-bar{padding:12px 28px;background:#FAF7F0;border-bottom:2px solid #EDE5C8;display:flex;gap:10px}'
    + '</style></head><body>'
    + '<div class="no-print no-print-bar">'
    + '<button onclick="window.print()" style="background:linear-gradient(135deg,#E2C47A,#C8A558);color:#0A0806;border:none;padding:10px 24px;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">🖨️ Imprimir / Guardar PDF</button>'
    + '<button onclick="window.close()" style="background:none;border:1px solid #EDE5C8;padding:10px 20px;border-radius:8px;font-size:14px;cursor:pointer;color:#5C4A1E">Cerrar</button>'
    + '</div>'
    + '<div class="hdr">'
    + '<div class="hdr-left">'
    + (logoSrc ? '<img class="hdr-logo" src="' + logoSrc + '" alt="DLC">' : '<div style="font-size:22px;font-weight:900;color:#E2C47A">DLC</div>')
    + '<div><div class="hdr-name">Distribuciones Estratégicas de la Costa S.A.S</div>'
    + '<div class="hdr-nit">NIT 901.445.281‑1</div>'
    + '<div class="hdr-info">☎ (57) 302 354 8415 &nbsp;&middot;&nbsp; distribucionesestrategicasco@gmail.com</div></div>'
    + '</div>'
    + '<div class="hdr-right"><div class="doc-type">Remisión de Despacho</div>'
    + '<div class="doc-num">' + o.id + '</div>'
    + '<div class="doc-date">' + today + '</div></div>'
    + '</div>'
    + '<div class="band"></div>'
    + '<div class="cli">'
    + '<div class="cli-box"><div class="cli-ttl">Datos del Cliente</div>'
    + '<div class="cli-row"><span class="cli-lbl">Cliente</span><span class="cli-val">' + (_esc(o.client) || '—') + '</span></div>'
    + '<div class="cli-row"><span class="cli-lbl">Empresa</span><span class="cli-val">' + (_esc(o.company) || '—') + '</span></div>'
    + '<div class="cli-row"><span class="cli-lbl">NIT / CC</span><span class="cli-val">' + (_esc(o.nit) || '—') + '</span></div>'
    + '<div class="cli-row"><span class="cli-lbl">F. Remisión</span><span class="cli-val">' + fmtFecha(o.date) + '</span></div>'
    + '</div>'
    + '<div class="cli-box"><div class="cli-ttl">Entrega / Contacto</div>'
    + '<div class="cli-row"><span class="cli-lbl">Email</span><span class="cli-val">' + (_esc(o.email) || '—') + '</span></div>'
    + '<div class="cli-row"><span class="cli-lbl">Teléfono</span><span class="cli-val">' + (_esc(o.phone) || '—') + '</span></div>'
    + '<div class="cli-row"><span class="cli-lbl">Ciudad</span><span class="cli-val">' + (_esc(o.city) || '—') + '</span></div>'
    + '</div></div>'
    + '<div class="tbl-hdr"><span class="tbl-ttl">Productos</span></div>'
    + '<div class="tbl-wrap"><table><thead><tr>'
    + '<th style="width:32px">#</th><th>Producto</th>'
    ...
    + '<div class="tot-row"><span>IVA (19%)</span><span>$' + fmt(iva) + '</span></div>'
    + '<div class="tot-total"><span>TOTAL</span><span>$' + fmt(total) + '</span></div>'
    + '</div></div>'
    + (o.notes ? '<div class="notes-box"><div class="notes-ttl">Observaciones</div><div style="font-size:12px;color:#5C4A1E">' + _esc(o.notes) + '</div></div>' : '')
    + '<div class="footer"><span class="ftr-l">Distribuciones Estratégicas de la Costa S.A.S &nbsp;&middot;&nbsp; distribucionesestrategicasco@gmail.com &nbsp;&middot;&nbsp; +57 302 354 8415</span><span class="ftr-r">' + o.id + '</span></div>'
    + '</body></html>';
...
function _buildRemisionHTML(datos) {
  ...
  var filas=items.map(function(item,i){
    ...
    return '<tr style="background:'+(i%2===0?'#fff':'#FAFAF8')+'">'
      +'<td style="padding:9px 10px;font-size:11px;color:#9A7535;border-bottom:1px solid #EDE5C8;text-align:center">'+(i+1)+'</td>'
      +'<td style="padding:9px 10px;font-size:13px;font-weight:600;color:#1A1208;border-bottom:1px solid #EDE5C8">'+_esc(item.name)+'</td>'
      +'<td style="padding:9px 10px;font-size:13px;font-weight:800;color:#9A7535;text-align:center;border-bottom:1px solid #EDE5C8">'+item.qty+'</td>'
      +(mostrarPrecios?'<td style="padding:9px 10px;font-size:12px;text-align:right;border-bottom:1px solid #EDE5C8;color:#5C4A1E">$'+fmt(item.price||0)+'</td>':'')
      +(mostrarPrecios?'<td style="padding:9px 10px;font-size:12px;font-weight:700;text-align:right;border-bottom:1px solid #EDE5C8;color:#1A1208">$'+fmt(sub2)+'</td>':'')
      +'<td style="padding:9px 10px;border-bottom:1px solid #EDE5C8;text-align:center"><div style="width:16px;height:16px;border:1.5px solid #C8A558;border-radius:2px;margin:0 auto"></div></td>'
    +'</tr>';
  }).join('');
  return '<div id="remision-print" style="font-family:Arial,sans-serif;background:#fff;font-size:13px">'
    +'<div style="padding:22px 28px;display:flex;align-items:center;border-bottom:1px solid #EDE5C8">'
      +'<div style="flex:1;padding-right:24px;border-right:2px solid rgba(200,165,88,0.28);display:flex;align-items:center;gap:12px">'+logo
        +'<div>'
          +'<div style="font-size:15px;font-weight:800;color:#1A1208;letter-spacing:-0.3px">Distribuciones Estrat\u00e9gicas de la Costa S.A.S</div>'
          +'<div style="font-size:9px;color:#9A7535;margin-top:2px">NIT 901.445.281-1 \u00b7 \u260e (57) 302 354 8415 \u00b7 distribucionesestrategicasco@gmail.com</div>'
          +'<div style="font-size:9px;color:#9A7535;margin-top:1px">Barranquilla, Colombia</div>'
        +'</div>'
      +'</div>'
      +'<div style="flex-shrink:0;text-align:right;padding-left:24px">'
        +'<div style="font-size:11px;font-weight:700;color:#9A7535;text-transform:uppercase;letter-spacing:1.5px">Remisi\u00f3n de Despacho</div>'
        +'<div style="font-size:22px;font-weight:900;color:#1A1208;letter-spacing:-0.5px;margin:2px 0">'+remNum+'</div>'
        +'<div style="font-size:10px;color:#5C4A1E">Fecha: '+today+'</div>'
      +'</div>'
    +'</div>'
    +'<div style="height:4px;background:#C8A558"></div>'
    +'<div style="display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid #EDE5C8">'
      +'<div style="padding:14px 22px;border-right:1px solid #EDE5C8">'
        +'<div style="font-size:9px;font-weight:700;color:#1A1208;text-transform:uppercase;letter-spacing:1.5px;padding-bottom:4px;border-bottom:2px solid #C8A558;display:inline-block;margin-bottom:10px">Datos del Cliente</div>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px">'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Cliente</div><div style="font-size:11px;font-weight:700;color:#1A1208">'+(_esc(cliente)||'&mdash;')+'</div></div>'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Empresa</div><div style="font-size:11px;font-weight:700;color:#1A1208">'+(_esc(empresa)||'&mdash;')+'</div></div>'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">NIT / CC</div><div style="font-size:11px;color:#1A1208">'+(_esc(nit)||'&mdash;')+'</div></div>'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Email</div><div style="font-size:11px;color:#1A1208">'+(_esc(email)||'&mdash;')+'</div></div>'
        +'</div>'
      +'</div>'
      +'<div style="padding:14px 22px">'
        +'<div style="font-size:9px;font-weight:700;color:#1A1208;text-transform:uppercase;letter-spacing:1.5px;padding-bottom:4px;border-bottom:2px solid #C8A558;display:inline-block;margin-bottom:10px">Entrega / Contacto</div>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px">'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Tel\u00e9fono</div><div style="font-size:11px;color:#1A1208">'+(_esc(telefono)||'&mdash;')+'</div></div>'
          +'<div><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Ciudad</div><div style="font-size:11px;color:#1A1208">'+(_esc(ciudad)||'&mdash;')+'</div></div>'
          +(orderId?'<div style="grid-column:1/-1"><div style="font-size:8px;color:#9A7535;font-weight:600;text-transform:uppercase;letter-spacing:0.8px">Pedido Ref.</div><div style="font-size:11px;color:#1A1208">'+_esc(orderId)+'</div></div>':'')
        +'</div>'
      +'</div>'
    +'</div>'
    ...
    +'<div style="padding:0 22px">'
      +'<table style="width:100%;border-collapse:collapse;font-size:12px">'
        +'<thead><tr style="background:#2A1C08">'
          ...
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:left;letter-spacing:1px;border-bottom:2px solid #C8A558">Producto</th>'
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:center;width:60px;letter-spacing:1px;border-bottom:2px solid #C8A558">Cant.</th>'
          +(mostrarPrecios?'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:right;width:90px;letter-spacing:1px;border-bottom:2px solid #C8A558">P. Unit.</th>':'')
          +(mostrarPrecios?'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:right;width:90px;letter-spacing:1px;border-bottom:2px solid #C8A558">Subtotal</th>':'')
          +'<th style="padding:8px 10px;font-size:9px;font-weight:700;color:#E2C47A;text-align:center;width:70px;letter-spacing:1px;border-bottom:2px solid #C8A558">Recibido \u2713</th>'
    ...
    +(mostrarTotales&&sub>0
      ?'<div style="padding:0 22px 12px;display:flex;justify-content:flex-end">'
        +'<div style="width:260px;border:1px solid #EDE5C8;border-top:none">'
          ...
          +'<div style="display:flex;justify-content:space-between;padding:9px 14px;font-size:13px;font-weight:900;background:#C8A558;color:#0A0806"><span>TOTAL</span><span>$'+fmt(total)+'</span></div>'
        +'</div></div>'
      :'')
    +(notas
    ...
    +'<div class="firmas-block" style="padding:10px 22px 14px;border-top:1px solid #EDE5C8;display:grid;grid-template-columns:1fr 1fr;gap:24px">'
      +'<div style="text-align:center">'
        +'<div style="font-size:9px;font-weight:700;color:#9A7535;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:6px">Firma Despachador</div>'
        +'<div style="border:1px solid #EDE5C8;border-top:3px solid #C8A558;background:#fff;min-height:70px;padding:6px 10px;display:flex;align-items:center;justify-content:center">'
        ...
        +'</div>'
        +'<div style="margin-top:5px;font-size:9px;color:#9A7535">Distribuciones Estrat\u00e9gicas de la Costa S.A.S</div>'
      +'</div>'
      +'<div style="text-align:center">'
        +'<div style="font-size:9px;font-weight:700;color:#9A7535;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:6px">Firma y Sello Receptor</div>'
        +'<div style="border:1px solid #EDE5C8;border-top:3px solid #C8A558;background:#fff;min-height:70px;padding:6px 10px"></div>'
        +'<div style="margin-top:5px;font-size:9px;color:#9A7535">Nombre, C.C. y Sello</div>'
      +'</div>'
    +'</div>'
    +'<div style="background:#0A0806;padding:8px 24px;display:flex;justify-content:space-between;align-items:center">'
      +'<div style="font-size:9px;color:rgba(226,196,122,0.50)">Generado el '+today+'</div>'
      +'<div style="font-size:9px;color:rgba(226,196,122,0.50)">'+remNum+'</div>'
    +'</div>'
  +'</div>';
}
