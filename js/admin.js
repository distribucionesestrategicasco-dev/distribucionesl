...
function renderPedidos() {
  ...
  const counts = {
    ...
    quoted:     all.filter(o => o.status === 'quoted').length,
  ...
  const tabs = [
    ...
    { key:'quoted',     label:'Cotizados',   count: counts.quoted },
  ...
  return `
    <div class="admin-header">
      <div>
        <h1>Pedidos</h1>
        <p>${all.length} pedido(s) en total</p>
      </div>
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px">
      ${tabs.map(t => `
        <button onclick="window._pedidosStatusFilter='${t.key}';renderLocalSection()" style="padding:8px 16px;border-radius:20px;border:2px solid ${statusFilter===t.key?'var(--brand-cyan)':'var(--border)'};background:${statusFilter===t.key?'var(--brand-cyan)':'transparent'};color:${statusFilter===t.key?'#fff':'var(--text)'};font-size:13px;font-weight:700;cursor:pointer">
          ${t.label} <span style="opacity:.8">(${t.count})</span>
        </button>
      `).join('')}
    </div>
    <div class="section-card">
      <div class="section-card-head"><h3>${statusFilter==='todos'?'Todos los Pedidos':'Pedidos: '+(STATUS_LABEL[statusFilter]||statusFilter)}</h3></div>
      ...
      ${filtered.length === 0
        ...
        : `<table>
            <thead>
              <tr><th>ID</th><th>Cliente</th><th>Contacto</th><th>Fecha</th><th>Estado</th><th>Productos / Historial</th><th>Acción</th></tr>
            </thead>
            <tbody>
              ${filtered.map(o => `
                <tr>
                  ...
                  <td>${fmtFecha(o.date)}</td>
                  <td><span class="badge ${STATUS_BADGE[o.status]||''}">${STATUS_LABEL[o.status]||o.status}</span></td>
                  <td>
                    <ul style="margin:0;padding-left:16px">${(o.items||[]).map(i => '<li style="font-size:13px">' + i.name + ' ×' + i.qty + '</li>').join('')}</ul>
                    ${renderHistorial(o)}
                  </td>
                  <td>
                    <button class="action-link" onclick="openQuotePanel('${o.id}')">Cotizar →</button>
                    ${currentUser && currentUser.rol === 'administrador' ? `
                      <button class="action-link" style="color:var(--brand-blue);margin-left:6px" onclick="editarPedido('${o.id}')">✏️</button>
                      <button class="action-link" style="color:#A32D2D;margin-left:4px" onclick="eliminarPedido('${o.id}')">🗑</button>
                    ` : ''}
                  </td>
                </tr>
