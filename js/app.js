...
// ── Dark mode (fijo en oscuro, sin toggle) ────
function initTheme() {
...
(function() {
  ...
  function _loadAndRender(order) {
    _supaFetch('/rest/v1/pedido_items?pedido_id=eq.' + encodeURIComponent(order.id))
      .then(function(r) { return r.json(); })
      .then(function(items) { order._items = items || []; window.renderOrder(order); })
      .catch(function() { order._items = []; window.renderOrder(order); });
  }
  ...
  window.renderOrder = function(order) {
    ...
    var stepsHTML = TRACKING_STEPS.map(function(s, i) {
      ...
      return '<div class="step">' +
        '<div class="step-icon' + (current ? ' current' : done ? ' done' : '') + '">' + (done && !current ? '✓' : s.icon) + '</div>' +
        '<div><div class="step-label' + (current ? ' current' : done ? ' done' : '') + '">' + s.label + '</div>' +
        (current ? '<div class="step-current-tag">● Estado actual</div>' : '') + '</div></div>';
    }).join('');
      ...
      itemsHTML = '<div class="info-box" style="margin-top:12px">' +
        ...
        order._items.map(function(i) {
          return '<div class="info-row">' +
            '<span class="lbl">' + (i.icon || '📦') + ' ' + i.name + '</span>' +
            '<span class="val">×' + i.qty + (i.price > 0 ? ' — $' + Math.round(i.price * i.qty).toLocaleString('es-CO') : '') + '</span></div>';
        }).join('') +
        (hasPrice ?
          '<div class="info-row" style="border-top:1px solid var(--border);margin-top:8px;padding-top:8px"><span class="lbl">Subtotal</span><span class="val">$' + Math.round(order.subtotal||0).toLocaleString('es-CO') + '</span></div>' +
          '<div class="info-row"><span class="lbl">IVA (19%)</span><span class="val">$' + Math.round(order.iva||0).toLocaleString('es-CO') + '</span></div>' +
          '<div class="info-row"><span class="lbl" style="font-weight:800">Total</span><span class="val" style="font-weight:800;color:var(--brand-blue)">$' + Math.round(order.total).toLocaleString('es-CO') + '</span></div>'
        : '') + '</div>';
    }
    ...
    if (st === 'quoted') {
      aprobarHTML = '<div style="margin-top:20px;padding:20px;background:rgba(0,196,167,0.08);border:1.5px solid rgba(0,196,167,0.35);border-radius:14px;text-align:center">' +
        '<p style="font-size:14px;color:var(--text);margin-bottom:14px;font-weight:600">¿Apruebas esta cotización?</p>' +
        '<button onclick="window.aprobarCotizacion(\'' + order.id + '\')"' +
          ' style="background:#00C4A7;color:#fff;border:none;padding:13px 32px;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;width:100%;max-width:300px">' +
          '✅ Aprobar Cotización</button></div>';
    }

    res.innerHTML =
      '<div class="order-header">' +
        '<div><div class="order-id">' + order.id + '</div>' +
        '<div class="order-sub">' + (order.client || '') + (order.company ? ' · ' + order.company : '') + '</div></div>' +
      ...
      '</div>' + stepsHTML + itemsHTML + aprobarHTML;
  };

  window.aprobarCotizacion = function(orderId) {
    if (!confirm('¿Confirmas que apruebas la cotización del pedido ' + orderId + '?')) return;
    ...
    .then(function(r) {
      ...
      return _supaFetch('/rest/v1/pedido_historial', {
        method: 'POST',
        body: JSON.stringify({ pedido_id: orderId, estado: 'Aprobado', fecha: new Date().toLocaleDateString('es-CO'), usuario: 'Cliente' }),
      });
    })
    .then(function() {
      document.getElementById('result').innerHTML =
        '<div style="text-align:center;padding:48px 20px">' +
          ...
          '<p style="color:var(--text-soft);font-size:13px;margin-top:20px">📞 ¿Tienes dudas? <strong>(57) 321 896 5745</strong></p>' +
        '</div>';
    })
    .catch(function(e) {
      console.error(e);
      if (btn) { btn.disabled = false; btn.textContent = '✅ Aprobar Cotización'; }
      alert('Error al aprobar. Inténtalo de nuevo.');
    });
  };
